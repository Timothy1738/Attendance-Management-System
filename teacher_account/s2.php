...
if (isset($_POST['select'])) {
    ...
    if ($period == 1) {
        if ($periodOneSubject == $subjectname) {
            $today = date('Y-m-d');
            $sql = "SELECT * FROM `attendancerecords` WHERE `Date` = '$today' AND `Period` = '$period' AND `ClassID` = 2";
...
if (isset($_SESSION['period'])) {
    if (isset($_POST['submit'])) {
        ...
        foreach ($_POST['attendance'] as $student_id => $attendance_data) {
            ...
            VALUES ('$student_id','$teacherID','$subject_id','$class_id','$date','$absent $present','$s2period')";

            // $sql = "INSERT INTO `attendancerecords`(`StudentID`, `TeacherID`, `ClassID`, `Date`, `IsPresent`, `Period`) VALUES
            // ('$student_id','$teacherID','$class_id','$date','$absent $present', '$s2period')";
    ...
    <section class="teacher_acc">
        ...
        <div class="container">
            ...
            <div class="viewATTadminTab">
                ...
                <a class="active" href="../teacher_account/s2.php">s2</a>
                        ...
                        <table>
                            ...
                            foreach ($gets2 as $s2) : ?>
                                <tr>
                                    ...
                                    <td><?php echo $s2['ClassName'] ?></td>
                                    <td class="present"><input type="checkbox" name="attendance[<?= $s2['StudentID'] ?>][present]" value="Present"> Present</td>
                                    <td class="absent"><input type="checkbox" name="attendance[<?= $s2['StudentID'] ?>][absent]" value="Absent"> Absent</td>
                                    <input type="hidden" name="class_id[<?= $s2['StudentID'] ?>]" value="<?= $s2['ClassID'] ?>">
