...
if (isset($_POST['select'])) {
    ...
    if ($period == 1) {
        if ($periodOneSubject == $subjectname) {
            $today = date('Y-m-d');
            $sql = "SELECT * FROM `attendancerecords` WHERE `Date` = '$today' AND `Period` = '$period' AND `ClassID` = 4";
            $res = mysqli_query($conn, $sql);
            if (mysqli_num_rows($res) > 0) {
                $error[] = "Attendance Already Been recorded for this period";
            } else {
                $_SESSION['s4_period'] = $period;
                $logic = 'SET';
            }
        } else {
...
if (isset($_SESSION['s4_period'])) {
    if (isset($_POST['submit'])) {
        ...
        foreach ($_POST['attendance'] as $student_id => $attendance_data) {
            ...
            VALUES ('$student_id','$teacherID','$subject_id','$class_id','$date','$absent $present','$s4period')";

            // $sql = "INSERT INTO `attendancerecords`(`StudentID`, `TeacherID`, `ClassID`, `Date`, `IsPresent`, `Period`) VALUES
            // ('$student_id','$teacherID','$class_id','$date','$absent $present', '$s4period')";
        ...
        if ($result) {
            ...
            $admin_msg = "Teacher" . ' ' . $fname . ' ' . $lname . ' ' . "has taken attendance for Class S4";
            $admin_state = 0;
    ...
    <section class="teacher_acc">
        ...
        <div class="container">
            ...
            <div class="viewATTadminTab">
                ...
                <a class="active" href="../teacher_account/s4.php">s4</a>
                ...
                if (!empty($logic)) {
                    ...
                    <form action="" id="attendanceForm" method="POST">
                        <table>
                            ...
                            <?php foreach ($gets4 as $s4) : ?>
                                <tr>
                                    <td><?php echo $s4['FirstName'] . ' ' . $s4['LastName'] ?></td>
                                    <td><?php echo $s4['ClassName'] ?></td>
                                    <td class="present"><input type="checkbox" name="attendance[<?= $s4['StudentID'] ?>][present]" value="Present"> Present</td>
                                    <td class="absent"><input type="checkbox" name="attendance[<?= $s4['StudentID'] ?>][absent]" value="Absent"> Absent</td>
                                    <input type="hidden" name="class_id[<?= $s4['StudentID'] ?>]" value="<?= $s4['ClassID'] ?>">
