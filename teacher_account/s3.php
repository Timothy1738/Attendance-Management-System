...
if (isset($_POST['select'])) {
    ...
    if ($period == 1) {
        if ($periodOneSubject == $subjectname) {
            $today = date('Y-m-d');
            $sql = "SELECT * FROM `attendancerecords` WHERE `Date` = '$today' AND `Period` = '$period' AND `ClassID` = 3";
            $res = mysqli_query($conn, $sql);
            if (mysqli_num_rows($res) > 0) {
                $error[] = "Attendance Already Been recorded for this period";
            } else {
                $_SESSION['s3_period'] = $period;
                $logic = 'SET';
            }
        } else {
...
if (isset($_SESSION['s3_period'])) {
    if (isset($_POST['submit'])) {
        ...
        foreach ($_POST['attendance'] as $student_id => $attendance_data) {
            ...
            VALUES ('$student_id','$teacherID','$subject_id','$class_id','$date','$absent $present','$s3period')";

            // $sql = "INSERT INTO `attendancerecords`(`StudentID`, `TeacherID`, `ClassID`, `Date`, `IsPresent`, `Period`) VALUES
            // ('$student_id','$teacherID','$class_id','$date','$absent $present', '$s3period')";
        ...
        if ($result) {
            ...
            $admin_msg = "Teacher" . ' ' . $fname . ' ' . $lname . ' ' . "has taken attendance for Class S3";
            $admin_state = 0;
    ...
    <section class="teacher_acc">
        ...
        <div class="container">
            ...
            <div class="viewATTadminTab">
                ...
                <a class="active" href="../teacher_account/s3.php">s3</a>
