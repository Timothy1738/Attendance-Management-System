...
if (isset($_POST['select'])) {
    ...
    if ($period == 1) {
        if ($periodOneSubject == $subjectname) {
            $today = date('Y-m-d');
            $sql = "SELECT * FROM `attendancerecords` WHERE `Date` = '$today' AND `Period` = '$period' AND `ClassID` = 6";
            $res = mysqli_query($conn, $sql);
            if (mysqli_num_rows($res) > 0) {
                $error[] = "You have already recorded Attendance for today";
            } else {
                $_SESSION['s6_period'] = $period;
                $logic = 'SET';
            }
        } else {
...
if (isset($_SESSION['s6_period'])) {
    if (isset($_POST['submit'])) {
        ...
        foreach ($_POST['attendance'] as $student_id => $attendance_data) {
            ...
            VALUES ('$student_id','$teacherID','$subject_id','$class_id','$date','$absent $present','$s6period')";

            // $sql = "INSERT INTO `attendancerecords`(`StudentID`, `TeacherID`, `ClassID`, `Date`, `IsPresent`, `Period`) VALUES
            // ('$student_id','$teacherID','$class_id','$date','$absent $present', '$s6period')";
        ...
        if ($result) {
            ...
            $admin_msg = "Teacher" . ' ' . $fname . ' ' . $lname . ' ' . "has taken attendance for Class S6";
            $admin_state = 0;
    ...
    <section class="teacher_acc">
        ...
        <div class="container">
            ...
            <div class="viewATTadminTab">
                ...
                <a class="active" href="../teacher_account/s6.php">s6</a>
                ...
                if (!empty($logic)) {
                    ...
                    <form action="" id="attendanceForm" method="POST">
                        <table>
                            ...
                            foreach ($gets6 as $s6) : ?>
                                <tr>
                                    <td><?php echo $s6['FirstName'] . ' ' . $s6['LastName'] ?></td>
                                    <td><?php echo $s6['ClassName'] ?></td>
                                    <td class="present"><input type="checkbox" name="attendance[<?= $s6['StudentID'] ?>][present]" value="Present"> Present</td>
                                    <td class="absent"><input type="checkbox" name="attendance[<?= $s6['StudentID'] ?>][absent]" value="Absent"> Absent</td>
                                    <input type="hidden" name="class_id[<?= $s6['StudentID'] ?>]" value="<?= $s6['ClassID'] ?>">
